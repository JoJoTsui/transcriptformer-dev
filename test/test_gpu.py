...
def test_select_gpus_filters_utilized_or_low_vram(monkeypatch) -> None:
    ...
    with mock.patch("pynvml.nvmlInit"), mock.patch(
        "pynvml.nvmlDeviceGetCount", return_value=3
    ), mock.patch(
        "pynvml.nvmlDeviceGetHandleByIndex", side_effect=lambda index: handles[index]
    ), mock.patch(
