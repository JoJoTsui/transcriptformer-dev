...
| 1.12 | **空间坐标准备工具已实现,原始文件尚未激活。** fig1、CS8、CS9 的坐标来自标识字符串;fig2 用 X_spatial,CS7 用 newx/newy。工具同时保留真实切片身份,避免原先清单常量合并多个切片。 | **已修复(工具)** — `fee6970`: `scripts/prepare_spatial_coordinates.py` 默认只读审计;显式复制模式输出新 H5AD 与派生清单,不改原始文件。412,374 个 spot 的全量坐标/元数据副本往返验证通过;16 项测试通过。完整表达矩阵副本已生成并通过派生清单校验(2026-09-23:27 PASS/1 WARN/0 FAIL,27 源有界演练通过);正式 prepare 仍待执行(5.5)。 |
...
| 4.3 | **各物种映射覆盖率曾为 47–82%(整改前数字,S6)。** 覆盖率差异会制造虚假的跨物种分化:某基因若不在物种 A 的映射中,看起来就像"被沉默"。整改后经真实代码路径由校验器实测为 52.7%(Tyser CS7)– 90.2%(线虫)——线虫从修复前的 47% 升至 90.2%。 | **已实现(表+实测覆盖)** — 2026-09-23 按原规则建成 `preprocess/orthologs/ortholog_pairs.tsv.gz`(402,495 对;91 个物种对中 68 对有数据;Ensembl Compara release 110/Metazoa 57 同源 TSV 导出——BioMart 被限流后按预案切换,来源与封锁记录已固定于 manifest):**仅 6 对通过双下限**(人×鼠、人×猪、人×猕猴、人×豚鼠、鼠×猪、鼠×豚鼠);海胆仅经 S. purpuratus 桥接连果蝇/线虫(`bridge_used`),文昌鱼在 Compara 无基因集(仅 B. lanceolatum 存在,未替代)。200 对抽查:OrthoDB 40 一致/21 不一致(已剔除)/139 不可用;Alliance DIOPT 35 一致/0 不一致。**下限现实待裁定:** 按既定规则,绝大多数跨物种基因级结论将降级为单物种发现;任何下限调整必须在查看结果之前决定。`c117bd3` |
...
| 7.7 | **B1 六物种留出标准不可达。** 7.3 要求 ≥6/8 训练物种改善,实际投影只有 human/mouse 两物种有 final_holdout;不能由 B4 探测物种补足分母。合作方第 1–3 项均不能改变此结论(两项语料候选均为小鼠数据;采样策略不影响留出资格)。 | **待决 — 训练前闸门** — `649b702` 的覆盖工具实测并明确输出 blocked,未修改标准。须在查看训练结果之前约定可评估物种范围/新标准,或补充独立胚胎;原阈值保留作历史记录。修订草案([b1-criterion-proposal.md](b1-criterion-proposal.md),B1-A/B1-B/B1-C 三案)已起草,待签字;训练开始即冻结。 |
...
**仍待执行。** 确定最终语料/QC配置并运行正式 prepare(完整表达矩阵副本已生成,派生清单校验通过,2026-09-23)。训练物种无空间留出;空间指标只能描述训练切片,独立探测评估仍受资源/元数据缺口阻塞。B1 标准另见 7.7。
...
| 4.3 | **Mapping coverage ranged 47–82% across species pre-fix (S6).** Differential coverage manufactures false cross-species divergence: a gene absent from species A's mapping looks "silenced". Post-fix, the validator-measured range through the real code path is 52.7% (Tyser CS7) – 90.2% (worm) — worm rose from 47% pre-fix to 90.2%. | **Implemented (table + measured coverage)** — 2026-09-23: `preprocess/orthologs/ortholog_pairs.tsv.gz` built under the original rules (402,495 pairs; 68 of 91 species pairs have data; Ensembl Compara release 110/Metazoa 57 homology TSV exports — the planned fallback after BioMart rate-limit blocking, with provenance and blockage history pinned in the manifest): **only 6 pairs pass both floors** (human×mouse, human×pig, human×macaque, human×guinea-pig, mouse×pig, mouse×guinea-pig); lytechinus reaches fly/worm only via the S. purpuratus bridge (`bridge_used`); amphioxus has no Compara gene set (only B. lanceolatum exists, explicitly not substituted). 200-pair spot check: OrthoDB 40 concordant / 21 discordant (dropped) / 139 unverified; Alliance DIOPT 35 / 0 / 1. **Floor reality check pending:** under the pre-registered rule almost all cross-species gene-level claims downgrade to single-species findings; any floor change must be decided before results are seen. `c117bd3` |
...
| 7.7 | **The six-species B1 gate is unachievable.** Item 7.3 requires improvement in ≥6/8 training species; the real projection provides holdout for only human/mouse. B4 probes cannot fill B1’s denominator. Collaborator decisions #1–#3 cannot change this (both corpus candidates are mouse-only; sampling policy does not affect holdout eligibility). | **Open — pre-training gate** — `649b702` measures coverage and explicitly reports blocked without changing the criterion. Before observing results, agree on evaluable species and a revised criterion or acquire independent embryos; the original threshold remains decision history. A revision draft ([b1-criterion-proposal.md](b1-criterion-proposal.md), options B1-A/B1-B/B1-C) is written and awaits sign-off; it freezes at training start. |
...
**Still pending.** Finalize corpus/QC configuration and run preparation (full expression copies are now created and their derived manifest validates, 2026-09-23). Training species have no spatial holdout; their spatial scores are descriptive, and independent probe evaluation still needs missing assets/metadata. See 7.7 for the separate B1 decision.
