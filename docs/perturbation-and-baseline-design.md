...
**2026-09-22 implementation audit — B1 blocked:** The [current coverage report](../logs/dataset_audit/holdout_coverage.json), using global `(species, embryo_id)` isolation across files, finds holdout embryos only in **human and mouse**. Six of eight training species lack holdouts, and all five spatial files are train-only; sections are not independent embryos. This is a pre-QC projection, so preparation may further reduce the measurable species × phase strata. The six-of-eight threshold below cannot be executed on this corpus. [Register item 7.7](finetune-major-issues.md) still requires a revised, agreed criterion or additional independent embryos before training. The original threshold remains for decision history; no replacement, including a two-species gate, has been approved. A pre-registration draft of the revised criterion (B1-A/B1-B/B1-C options with sign-off record) is in [b1-criterion-proposal.md](b1-criterion-proposal.md); it is a proposal only and freezes at training start.
...
**2026-09-23 build status:** the table is built ([manifest](../preprocess/orthologs/manifest.json), [coverage report](../logs/dataset_audit/orthologs/coverage_report.json), commit `c117bd3`) on Compara release 110 / Metazoa 57 via the homology TSV exports (the planned fallback after BioMart rate-limit blocking): 402,495 1:1 pairs across 68 of 91 species pairs, with only 6 pairs passing both floors. Floor definitions remain as pre-registered; changing them is a pre-unblinding decision.

For probe species, tokens must be built from ESM2 protein embeddings for the exact source species; `preprocess/fasta_manifest_pep.json` is currently incomplete for these probes (see §3 readiness status). Gene-level cross-species statements use the same 1:1 table; embedding-level comparisons do not require it.
